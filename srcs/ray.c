...
t_ray	*cam_ray(t_cam *cam, t_rt *rt, double x, double y)
{
	...
	vec1 = mul_vec(cam->right_vec, u);
	vec2 = mul_vec(cam->up_vec, v);
	vec3 = add_vec(vec1, vec2);
	tray->direction = add_vec(cam->corner_vec, vec3);
	tray->direction = sub_vec(tray->direction, cam->coords);
	normalize_vec(tray->direction);
	// printf("u, v: %lf, %lf\n", u, v);
	// printf("tray origin vector: %lf, %lf, %lf\n", tray->origin->x , tray->origin->y, tray->origin->z);
	// printf("tray direction vector: %lf, %lf, %lf\n", tray->direction->x , tray->direction->y, tray->direction->z);
	return (tray);
}
